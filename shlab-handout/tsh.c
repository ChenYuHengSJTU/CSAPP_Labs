...
void eval(char *cmdline) 
{
    ...
    if(!builtin_cmd(Argv)){
        ...
        if(cpid == 0){
            ...
            addjob(jobs, cpid, flag + 1, cmdline);
            if (sigprocmask(SIG_UNBLOCK, &mask, NULL) < 0) {
                perror("sigprocmask");
                exit(EXIT_FAILURE);
            }
            // jobs will be delete through sigchld handler
            if(!flag){
                waitfg(cpid);
                // deletejob(jobs, cpid);
            }
...
void sigchld_handler(int sig) 
{
    ...
    fflush(stdout);
    int status;
    pid_t wpid = waitpid(fgpid(jobs), &status, WNOHANG | WUNTRACED);
...
void sigint_handler(int sig) 
{
    assert(fgpid(jobs) != getpid() || fgpid(jobs) != 0);
